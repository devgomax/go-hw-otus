...
type IApp interface {
	CreateEvent(ctx context.Context, title, description, userID string, startsAt, endAt *time.Time, notifyInterval time.Duration) error
	...
	UpdateEvent(ctx context.Context, id, title, description, userID string, startsAt, endAt *time.Time, notifyInterval time.Duration) error
}
